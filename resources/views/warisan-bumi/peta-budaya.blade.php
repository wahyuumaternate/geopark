...
@section('title', 'Peta Warisan Budaya | Geopark Ternate')
@section('meta_description', 'Peta interaktif sebaran situs warisan budaya di Geopark Ternate.')
...
@section('page_title', 'Peta Warisan Budaya')
...
@push('styles')
    ...
    <style>
        ...
        .budaya-list .item-card:hover,
        .budaya-list .item-card.active {
            background: #17a2b8;
            color: #fff;
            border-color: #17a2b8;
        }

        .budaya-list .item-card h6 {
            margin-bottom: 4px;
        }

        .budaya-list .item-card small {
            opacity: 0.8;
        }
        ...
        <div class="container">
            <div class="row justify-content-center pb-4">
                <div class="col-md-9 text-center heading-section ftco-animate">
                    ...
                    <h2 class="mb-4">Peta Sebaran Warisan Budaya</h2>
                    <p>Jelajahi lokasi situs, tradisi, dan warisan budaya Geopark Ternate melalui peta interaktif di bawah
                        ini.</p>
                </div>
            </div>
            ...
            <div class="row ftco-animate">
                ...
                <div class="col-md-4 mb-4 mb-md-0">
                    <div class="budaya-list" id="budayaList">
                        @forelse ($items as $item)
                            <a href="#" class="item-card" data-id="{{ $item->id }}" data-lat="{{ $item->y }}"
                                data-lng="{{ $item->x }}">
                                <h6>{{ $item->nama }}</h6>
                                <small>{{ $item->lokasi ?? ($item->kelurahan ?? '-') }}</small>
                            </a>
                        @empty
                            <p class="text-muted">Belum ada data warisan budaya dengan koordinat.</p>
                        @endforelse
                    </div>
                </div>
        ...
        document.addEventListener('DOMContentLoaded', function() {
            ...
            document.querySelectorAll('#' + listId + ' .item-card').forEach(function(card) {
                card.addEventListener('click', function(e) {
                    ...
                    document.querySelectorAll('#' + listId + ' .item-card').forEach(c => c.classList
                        .remove('active'));
